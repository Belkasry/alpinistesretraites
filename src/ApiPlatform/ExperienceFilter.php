    ...
    class ExperienceFilter extends AbstractFilter
    {
        ...
        protected function filterProperty(string $property, $value, QueryBuilder $queryBuilder, QueryNameGeneratorInterface $queryNameGenerator, string $resourceClass, string $operationName = null/*, array $context = []*/)
        {
            ...
            if ($property == 'all') {
                ...
                $queryBuilder->andWhere(sprintf('%s.title LIKE :%s OR %s.description LIKE :%s', $alias, $valueParameter.$key, $alias, $valueParameter.$key))
                    ->setParameter($valueParameter.$key, '%' . $val . '%');
                }

            } elseif ($property == 'title') {
                $alias = $queryBuilder->getRootAliases()[0];
                $valueParameter = $queryNameGenerator->generateParameterName('title');
                $searchValues = explode(" ", $value);
                foreach ($searchValues as $key=>$val){
                    $queryBuilder->andWhere(sprintf('%s.title LIKE :%s ', $alias, $valueParameter.$key, $alias, $valueParameter.$key))
                        ->setParameter($valueParameter.$key, '%' . $val . '%');
                }
                ...
                $searchValues = explode(" ", $value);
                foreach ($searchValues as $key=>$val){
                    $queryBuilder->andWhere(sprintf('%s.guide = :%s ', $alias, $valueParameter.$key, $alias, $valueParameter.$key))
                        ->setParameter($valueParameter.$key,  $val );
                }
            }
             else
